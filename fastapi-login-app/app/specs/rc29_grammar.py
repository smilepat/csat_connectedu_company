...
class RC29Spec(ItemSpec):
    ...
    def quote_postprocess(self, passage: str, llm_json: dict) -> dict:
        ...
        # ✅ (A) 타겟 텍스트를 실제 passage 안의 표현으로 "스냅"
        resolved_texts: list[str] = []

        for t in texts:
            t_clean = (t or "").strip()
            if not t_clean:
                resolved_texts.append(t_clean)
                continue

            # 1) 완전 일치 먼저 검사
            pat_exact = re.compile(rf"\b{re.escape(t_clean)}\b", re.I)
            if pat_exact.search(passage):
                resolved_texts.append(t_clean)
                continue

            # 2) 부분 구간(서브 프레이즈)에서 실제로 존재하는 표현 찾기
            words = t_clean.split()
            found_phrase = None

            # 길이가 긴 구간부터 짧은 구간 순으로 탐색
            # (최대 4단어까지만 사용하면 이후 1~4 토큰 규칙과도 잘 맞음)
            max_len = min(len(words), 4)
            for length in range(max_len, 0, -1):
                for start in range(0, len(words) - length + 1):
                    cand = " ".join(words[start:start + length])
                    pat_cand = re.compile(rf"\b{re.escape(cand)}\b", re.I)
                    if pat_cand.search(passage):
                        found_phrase = cand
                        break
                if found_phrase:
                    break
            ...
            if found_phrase:
                # 예: "to be succeeded" → "succeeded"
                #     "were soon succeeded" 같은 게 있으면 그 구간 전체로 스냅
                resolved_texts.append(found_phrase)
            else:
                # 그래도 아무 구간도 못 찾으면 경고만 남기고 원본 유지
                print(f"[RC29 quote] Warning: target '{t_clean}' not found in passage; using as-is.")
                resolved_texts.append(t_clean)

        texts = resolved_texts

        # ---- 2) 카테고리 검사 (신형 포맷일 때만 강제) ----
        allowed_cats = {"relative", "tense_or_agreement", "modal", "passive", "participle"}
        if categories:
            # 허용된 카테고리만 필터링
            valid_cats = [c for c in categories if c in allowed_cats]
            unknown_cats = [c for c in categories if c and c not in allowed_cats]

            if unknown_cats:
                # 예: ["infinitive"] 같은 값들 → 경고만 남기고 진행
                print(f"[RC29 quote] Warning: unknown grammar categories ignored: {unknown_cats}")

            # 최소 3개의 유효 카테고리가 있어야 함
            if len(valid_cats) < 3:
                raise ValueError(
                    "RC29(quote): need at least 3 valid grammar categories "
                    f"from {allowed_cats}, got {valid_cats}"
                )

            # 서로 다른 범주도 최소 3개 이상이어야 함
        ...
        for m in marks:
            ...
            if not (1 <= len(tokens) <= 4):
                raise AssertionError(
                    ...
                    f"{len(tokens)} tokens; require 1–4."
                )